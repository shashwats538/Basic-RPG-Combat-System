...
int main()
{
    ...
    // The player
    vector<Attack> playerAttacks = { {"Sword To The Face", 20} };
    vector<Item> playerItems = { {"Old Grog", 50} };
    Character player(playerName, 2, 2, playerAttacks, playerItems);
    ...
    bool gameOver = false;
    cout << playerName <<" finds himself trapped in a scary dungeon! There seems to be a series of rooms, he enters the first room..." << endl;
    while(!monsters.empty() && !gameOver)
    {
        ...
        while(!currentMonster.isDead())
        {
            ...
            if(playerTurn)
            {
                ...
                cout << "Player can press 1 and enter to use an item and 2 and enter to attack the monster." << endl;
                ...
                while(!madeChoice)
                {
                    ...
                    {
                        case 1:
                            player.UseItem("Old Grog");
                            madeChoice = true;
                            break;
                            
                        case 2:
                            player.DoAttack(currentMonster);
                            madeChoice = true;
                            break;
            ...
            else
            {
                ...
                currentMonster.DoAttack(player);
            }
            
            cout << playerName<<"'s health is " << player.getHealth() << endl;
